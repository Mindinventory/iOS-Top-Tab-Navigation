...
extension TopTabbarView {
    ...
    private func performSpringAnimation(for cell: ItemCollectionViewCell, index: Int) {
        UIView.animate(withDuration: 0.75, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.0, options: [], animations: { () -> Void in
            self.collectionView.update(with: index)
            
        }, completion: { _ in
            self.collectionView.animating = false
        })
        UIView.animate(withDuration: 0.75, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.0, options: .curveEaseInOut, animations: {
            if self.isScaleItem {
    ...
    public func setSelectedTab(with index: Int) {
        
        if index != self.priviousSelectedIndex {
            
            collectionView.animating = true
            ...
            priviousSelectedIndex = index
            guard let cell = collectionView.cellForItem(at: [0,index]) as? ItemCollectionViewCell
            else { return }
            performSpringAnimation(for: cell, index: index+1)
        }
    }
}
...
extension TopTabbarView: UICollectionViewDelegate,
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        setSelectedTab(with: indexPath.row)
    }
}
